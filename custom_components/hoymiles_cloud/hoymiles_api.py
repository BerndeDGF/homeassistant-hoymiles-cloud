...
class HoymilesAPI:
    ...
    async def get_microinverters_by_stations(self, station_id: str) -> Dict[str, str]:
        """Get all microinverters for a station."""
        if not self._token:
            _LOGGER.debug("No token available, authenticating first")
            await self.authenticate()
            ...
            async with self._session.post(
                ...
                if resp.get("status") == "0" and resp.get("message") == "success":
                    ...
                    for microinverter in microinverters_data:
                        ...
                        _LOGGER.debug("Adding microinverters: %s - %s", microinverter_id, microinverter)
                        microinverters[microinverter_id] = microinverter
                        
                    _LOGGER.debug("Returning microinverters dictionary: %s", stations)
